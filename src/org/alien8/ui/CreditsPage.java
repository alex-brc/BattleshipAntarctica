...
public class CreditsPage implements Page {
  ...
  public CreditsPage() {
    ...
    String text = "CREDITS";
    creditsTitle = new InfoBox(r.getWidth() / 2 - text.length() * 8, r.getHeight() / 9 - 8,
        text.length() * 16, 16);
    creditsTitle.updateText(text);

    text = "many thanks to:";
    sfxCredits = new InfoBox(r.getWidth() / 2 - text.length() * 8, r.getHeight() / 9 * 3 - 8,
        text.length() * 16, 16);
    sfxCredits.updateText(text);
    text = "LittleRobotSoundFactory for sound effects";
    sfxCredits2 = new InfoBox(r.getWidth() / 2 - text.length() * 8, r.getHeight() / 9 * 4 - 8,
        text.length() * 16, 16);
    sfxCredits2.updateText(text);
    text = "- freesound.org -";
    sfxCredits3 = new InfoBox(r.getWidth() / 2 - text.length() * 8, r.getHeight() / 9 * 4 + 12,
        text.length() * 16, 16);
    sfxCredits3.updateText(text);
    text = "OZZED for his great music";
    musicCredits = new InfoBox(r.getWidth() / 2 - text.length() * 8, r.getHeight() / 9 * 5 - 8,
        text.length() * 16, 16);
    musicCredits.updateText(text);
    text = "- ozzed.net -";
    sfxCredits4 = new InfoBox(r.getWidth() / 2 - text.length() * 8, r.getHeight() / 9 * 5 + 12,
        text.length() * 16, 16);
    sfxCredits4.updateText(text);
    text = "and every member of Alien8";
    sfxCredits5 = new InfoBox(r.getWidth() / 2 - text.length() * 8, r.getHeight() / 9 * 6 - 8,
        text.length() * 16, 16);
    sfxCredits5.updateText(text);
    text = "- the code is alien8 -";
    sfxCredits6 = new InfoBox(r.getWidth() / 2 - text.length() * 8, r.getHeight() / 9 * 7 - 8,
        text.length() * 16, 16);
    sfxCredits6.updateText(text);

    returnBtn = new ReturnToMainButton(r.getWidth() / 2 - btnWidth, r.getHeight() / 9 * 8 - 10,
        2 * btnWidth, 40);
  }
